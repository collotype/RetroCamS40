...
enum RetroFilter {
    ...
    static func makeRetroPhoto(
        ...
        let prepared = preparePhotoBase(image, preset: preset, aspect: aspect)
    ...
    static func normalizePhoto(
        ...
        var result = image.normalized()

        if aspect == .fourThree {
            result = result.centerCroppedTo4by3()
        }
    ...
    static func applyToVideoFrame(_ image: CIImage, preset: RetroPreset) -> CIImage {
        applyRetro(to: image, preset: preset)
    }
    ...
    private static func preparePhotoBase(
        ...
        var base = image.normalized()

        if aspect == .fourThree {
            base = base.centerCroppedTo4by3()
        }
    ...
    private static func applyPointAndShoot(to image: CIImage) -> CIImage {
        ...
        if let noise = CIFilter(name: "CIRandomGenerator")?.outputImage?.cropped(to: extent) {
            let grain = noise.applyingFilter("CIColorMatrix", parameters: [
                "inputAVector": CIVector(x: 0, y: 0, z: 0, w: 0.015)
            ])
